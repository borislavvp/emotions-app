...
def convert(pixels) :
    data = [[0 for x in range(int(oledHeight/8))] for x in range(oledWidth)]

    for i in range(oledWidth):
        for j in range(int(oledHeight/8)):
            for bit in range(8):
    ...
    for j in range(oledHeight):
        for i in range(oledWidth):
            ...
            if isinstance(pixel_value, tuple):
                value = pixel_value[0]  # Accessing the R component of the RGB tuple
            else:
                value = pixel_value 
